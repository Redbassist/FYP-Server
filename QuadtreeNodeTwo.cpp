...
QuadtreeNodeTwo::QuadtreeNodeTwo(const sf::FloatRect &region, int level, QuadtreeNodeTwo* pParent, Quadtree* pQuadtree)
    : _pParent(pParent)
...
void QuadtreeNodeTwo::create(const sf::FloatRect &region, int level, QuadtreeNodeTwo* pParent, Quadtree* pQuadtree) {
    _hasChildren = false;
...
void QuadtreeNodeTwo::getPossibleOccupantPosition(QuadtreeOccupant* oc, sf::Vector2i &point) {
...
void QuadtreeNodeTwo::addToThisLevel(QuadtreeOccupant* oc) {
    oc->_pQuadtreeNodeTwo = this;
...
bool QuadtreeNodeTwo::addToChildren(QuadtreeOccupant* oc) {
    assert(_hasChildren);
    ...
    QuadtreeNodeTwo* pChild = _children[position.x + position.y * 2].get();
...
void QuadtreeNodeTwo::partition() {
    assert(!_hasChildren);
    ...
    for (int x = 0; x < 2; x++)
        for (int y = 0; y < 2; y++) {
            ...
            _children[x + y * 2].reset(new QuadtreeNodeTwo(childAABB, nextLowerLevel, this, _pQuadtree));
        }

    _hasChildren = true;
}

void QuadtreeNodeTwo::merge() {
    if (_hasChildren) {
...
void QuadtreeNodeTwo::getOccupants(std::unordered_set<QuadtreeOccupant*> &occupants) {
...
void QuadtreeNodeTwo::removeForDeletion(std::unordered_set<QuadtreeOccupant*> &occupants) {
    // Iteratively parse subnodes in order to collect all occupants below this node
    std::list<QuadtreeNodeTwo*> open;
...
void QuadtreeNodeTwo::getAllOccupantsBelow(std::vector<QuadtreeOccupant*> &occupants) {
    // Iteratively parse subnodes in order to collect all occupants below this node
    std::list<QuadtreeNodeTwo*> open;
...
void QuadtreeNodeTwo::getAllOccupantsBelow(std::unordered_set<QuadtreeOccupant*> &occupants) {
    // Iteratively parse subnodes in order to collect all occupants below this node
    std::list<QuadtreeNodeTwo*> open;
...
void QuadtreeNodeTwo::update(QuadtreeOccupant* oc) {
    if (oc == nullptr)
        return;
    ...
    if (pNode == nullptr) {
        ...
        oc->_pQuadtreeNodeTwo = nullptr;
    }
    else // Add to the selected node
        pNode->add(oc);
}

void QuadtreeNodeTwo::remove(QuadtreeOccupant* oc) {
...
void QuadtreeNodeTwo::add(QuadtreeOccupant* oc) {
    assert(oc != nullptr);
