...
namespace ltbl {
    class QuadtreeNodeTwo : public sf::NonCopyable {
    ...
    private:
        QuadtreeNodeTwo* _pParent;
        class Quadtree* _pQuadtree;

        bool _hasChildren = false;

        std::array<std::unique_ptr<QuadtreeNodeTwo>, 4> _children;
    ...
    public:
        ...
        QuadtreeNodeTwo(const sf::FloatRect &region, int level, QuadtreeNodeTwo* pParent, class Quadtree* pQuadtree);

        // For use after using default constructor
        void create(const sf::FloatRect &region, int level, QuadtreeNodeTwo* pParent, class Quadtree* pQuadtree);
